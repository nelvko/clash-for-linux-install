...
_require_empty_home "$legacy" >"$WORK_DIR/legacy.stdout" 2>"$WORK_DIR/legacy.stderr" &&
    fail 'legacy layout without a marker was accepted'
[ "$_INSTALL_HOME_STATE" != resume ] || fail 'legacy layout must not enter resume state'
grep -Fqs '旧版目录' "$WORK_DIR/legacy.stderr" ||
    fail 'legacy rejection lacks the migration guidance'
